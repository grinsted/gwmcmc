function [models,logP]=gwmcmc_par(minit,logPfuns,mccount,stepsize,skip)
...
for ii=2:mccount
    ...
    logrand=log(rand(NPfun+1,Nwalkers));
    parfor wix=1:Nwalkers %TODO:PARFOR
        lr=logrand(:,wix);
        cp=curlogP(:,wix);
...
progress(1,curm,sum(reject)/(ii*Nwalkers));
